...
describe Configuration do
	it 'correctly parses a valid configuration file' do
		cfg = Configuration.parse(File.join(__dir__, 'files/valid.yaml'))

		expect(cfg[:content_dir]).to eq('content')
		expect(cfg[:collections].length).to eq(2)
		expect(cfg[:collections][0][:id]).to eq('poem')
		expect(cfg[:collections][0][:label]).to eq('Poems')
		expect(cfg[:collections][0][:label_singular]).to eq('Poem')
	...
	it 'uses the default location if not explicitly set' do
		cfg = Configuration.parse(File.join(__dir__, 'files/no_location.yaml'))

		expect(cfg[:collections][0][:location]).to eq('/')
	end

	it 'uses the default filetype if not explicitly set' do
		cfg = Configuration.parse(File.join(__dir__, 'files/no_filetype.yaml'))
	...
	it 'complains if no collections are given' do
		expect do
			Configuration.parse(File.join(__dir__, 'files/no_collections.yaml'))
		end.to raise_error(JSON::Schema::ValidationError)
	end

	it 'complains if an invalid field type is given' do
		expect do
			Configuration.parse(File.join(__dir__, 'files/invalid_field_type.yaml'))
		end.to raise_error(JSON::Schema::ValidationError)
	end

	it 'complains if a required field attribute is missing' do
		expect do
			Configuration.parse(File.join(__dir__, 'files/missing_required_field_attr.yaml'))
		end.to raise_error(JSON::Schema::ValidationError)
	end

	it 'complains if the filetype is invalid' do
		expect do
			Configuration.parse(File.join(__dir__, 'files/invalid_file_type.yaml'))
		end.to raise_error(JSON::Schema::ValidationError)
	end

	it 'complains if a required collection attribute is missing' do
		expect do
			Configuration.parse(File.join(__dir__, 'files/missing_required_collection_attr.yaml'))
		end.to raise_error(JSON::Schema::ValidationError)
	end

	it 'complains if multiple richtext fields are added in one collection' do
		expect do
			Configuration.parse(File.join(__dir__, 'files/multiple_richtext.yaml'))
		end.to raise_error(RobinCMS::Configuration::ValidationError)
	end
